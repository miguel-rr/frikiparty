 ...
 * state in that case. Pure query; pages using it revalidate hourly since
 * "today" is baked in at build time.
 */
const getNextEdition = async (db: TRPCContext['db']) => {
  const today = new Date().toISOString().slice(0, 10);
  const [row] = await db
    .select({
      id: edition.id,
 ...
 * the team championship, the size-1 team the individual one. Pure query.
 */
...
const editionRouter = createTRPCRouter({
  next: publicProcedure.query(({ ctx }) => getNextEdition(ctx.db)),
