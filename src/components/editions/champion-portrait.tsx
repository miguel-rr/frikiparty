...
const SIZES = {
  sm: { box: 'size-9', ring: 'size-3', text: 'text-xs' },
...
const ChampionPortrait = ({
  ...
  const face = champion.portrait ? (
    <span className={`relative block ${s.box} shrink-0`}>
      ...
      <span
        aria-hidden
        className="pointer-events-none absolute top-[-45%] -right-2/5 bottom-1/2 -left-2/5 overflow-hidden"
      >
        {/* biome-ignore lint/performance/noImgElement: local static portrait */}
        <img
          alt=""
